...
var invSuccess = function(data, user_index) {
  user_data[users[user_index]].bagCandy = filter(data, 'candy');
...
function getCandy(p_num, user_id) {
    var self = this,
        user = self.user_data[users[user_id]];
  ...
  //for (var i = 0; i <  user_data[users[user_id]].bagCandy.length; i++) {
    //var checkCandy = user_data[users[user_id]].bagCandy[i].inventory_item_data.candy.family_id;
    //if (pokemoncandyArray[p_num] === checkCandy) {
      //return (user_data[users[user_id]].bagCandy[i].inventory_item_data.pokemon_family.candy.candy || 0);
    //}
  //}
}
